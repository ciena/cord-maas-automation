...
func ProcessAll(client *maas.MAASObject, nodes []MaasNode, include string) []error {
	errors := make([]error, len(nodes))
	filter, err := regexp.Compile(include)
	if err != nil {
		log.Fatalf("[error] invalid regular expression for include filter '%s' : %s", include, err)
	}
	for i, node := range nodes {
		if filter.MatchString(node.Hostname()) {
			...
			log.Printf("[info] ignoring node '%s' as it didn't match include filter '%s'", node.Hostname(), include)
		}
	}
	return errors
}
